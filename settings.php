<div class="wrap">
    ...
    <h4>The Auphonic Importer will read the last 10 Productions from the Auphonic API once every hour.<br>
        If you need to update manually, click <a href="./?ai_update=<?= $options['ai_update_password'] ?>">here</a></h4>
    ...
    <form method="post" action="options-general.php?page=ai-settings">
        ...
        Your update url is: <?php echo "http://" . $_SERVER['HTTP_HOST'] . "/?ai_update=" . $options['ai_update_password']; ?><br><br>
        <b>Tag</b><br><input type="text" name="ai_tag" value="<?= $options['ai_tag'] ?>" size="50">
        <p>The tag corresponds to the tag you put in on you preset. If you use Auphonic for more
            than one production, this will filter it and only import productions from one preset.</p>

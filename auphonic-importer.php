...
function ai_read_auphonic() {
    ...
    if ($options['ai_enabled'] == "Yes") {
        if ($options['ai_username'] != "" && $options['ai_password'] != "" && $options['ai_tag'] != "") {
            ...
            $query = "select filename from " . $wpdb->prefix . "auphonic_importer";
            $rows = $wpdb->get_results($query);

            $previousPodcasts = array();
            foreach ($rows as $row) {
                $previousPodcasts[] = $row->filename;
            }
                ...
                foreach ($output['data'] as $production) {
                    ...
                    if ($production['status_string'] == "Done") {
                        if (in_array($tag, $tags)) {
                            ...
                            if (isset($enclosure) && !in_array($url, $previousPodcasts)) {
